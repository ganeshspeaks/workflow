    ...
    if (active_board_id != null && columns != null) {
      axios.post('https://api.monday.com/v2', {
        ...
        if (active_board_items.length > 0) {
          ...
          let item_names = active_board_items.map(item => item.name);
          setActiveBoard(item_idss);
          setItemsNames(item_names);
                        ...
                        <thead>
                          <tr>
                            {columns?.length > 0 && columns?.map((column) => {
                              return (
                                <th className="text-center w-fit align-middle border border-solid py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold bg-pink-800 text-pink-300 border-pink-700">{column?.title}</th>
                              )
                            })}
                          </tr>
                        </thead>
                          ...
                          {active_board?.length > 0 && (
                            // console.log(active_board, active_board_id,items_names)
                              <TableRow board_id={active_board_id} item_ids={active_board} item_names={items_names} length={columns?.length} />
                          )}
